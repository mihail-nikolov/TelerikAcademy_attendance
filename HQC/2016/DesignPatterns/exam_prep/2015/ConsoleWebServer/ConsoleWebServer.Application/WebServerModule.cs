...
namespace ConsoleWebServer.Application
{
    public class WebServerModule : NinjectModule
    {
        ...
        public override void Load()
        {
            ...
            Bind<IHttpResponseFactory>().ToFactory().InSingletonScope();
            Bind<IActionResultFactory>().ToFactory().InSingletonScope();
            //Bind<IHttpRequestFactory>().ToFactory().InSingletonScope(); // the same as the response factory
            ...
            Bind<IActionResult>()
                .ToMethod(context =>
                {
                    ...
                    return context.Kernel.Get<IActionResult>(ActionResultWithCorsName, contextParams[2],
                        new ConstructorArgument(ActionResultConstructorArgument, context.Kernel.Get<IActionResult>(JsonActionResultName,
                        contextParams[0], contextParams[1])));
            ...
            Bind<IActionResult>()
                .ToMethod(context =>
                {
                    List<IParameter> contextParams = context.Parameters.ToList();
                    return context.Kernel.Get<IActionResult>(ActionResultWithCorsName, contextParams[2],
                        new ConstructorArgument(ActionResultConstructorArgument, context.Kernel.Get<IActionResult>(ContentActionResultWithNoCachingName,
                        contextParams[0], contextParams[1])));
